...
def first_time_signup(request):
    ...
    #IF they submitted a vbb email, try to use that email, ELSE create a new vbb email for them
    if request.data["vbb_email"] is not None and request.data["vbb_email"] != '':
        ...
        pwd = "the same as it has been."
    else:
        # check to see if the serializer works, using a filler vbb email for now
        ...
        #if the serializer is happy with the data they've submitted, then create a new google account for them
        request.data["vbb_email"], pwd = gapi.account_create(
            fname.lower(), lname.lower(), pemail
        )

    #Generate the welcome email using their information and send it to them using the google API
    ...
    gapi.email_send(
        ...
        "Welcome to the VBB Mentoring Program!",
        welcome_mail,
        {
    ...
    #subscribe their personal and vbb emails to the google groups
    ...
    #deserialize their registration information and save it to the database
    serializer = MentorProfileSerializer(data=request.data)
